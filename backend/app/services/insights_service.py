...
class InsightsService:
    ...
    def generate_developer_insights(
        ...
        insights.extend(
            self._detect_productivity_patterns(developer, start_date, end_date)
        )
        insights.extend(
            self._detect_work_style_patterns(developer, start_date, end_date)
        )
    ...
    def _detect_productivity_patterns(
        ...
        avg_older = (
            sum(recent_scores[2:]) / 2 if len(recent_scores) >= 4 else avg_recent
        )
    ...
    def _detect_work_style_patterns(
        ...
        complexity_scores = [
            a.complexity_score for a in activities if a.complexity_score is not None
        ]
        avg_complexity = (

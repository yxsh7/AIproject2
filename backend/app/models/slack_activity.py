...
class SlackMessage(Base):
    ...
    __table_args__ = (UniqueConstraint("message_ts", name="uq_slack_message_ts"),)
    ...
    )  # Slack timestamp — globally unique; uniqueness enforced by uq_slack_message_ts constraint
    ...
    developer = relationship(
        "DeveloperProfile", foreign_keys=[developer_id], back_populates="slack_messages"
    )
...
class SlackReaction(Base):
    ...
    target_message_ts = Column(
        String, nullable=False
    )  # No FK — reactions may reference messages in unmonitored channels
    ...
    developer = relationship(
        ...
        back_populates="slack_reactions",
    )
